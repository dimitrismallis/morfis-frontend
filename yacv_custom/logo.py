...
ASSETS_DIR = os.getenv('ASSETS_DIR', os.path.join(
...
def build_logo(text: bool = True) -> Dict[str, Union[Compound, Location, str]]:
    ...
    with BuildPart(Plane.XY.offset(50)) as logo_obj:
        ...
        offset(solid(), 2, openings=faces().group_by(
            Axis.Z)[0] + faces().filter_by(Plane.XZ))
        if text:
            with BuildSketch(Plane.YZ.move(Pos(faces().group_by(Axis.X)[-1].face().center()))):
                Text('Yet Another\nCAD Viewer', 6,
                     font_path='/usr/share/fonts/TTF/Hack-Regular.ttf')
            extrude(amount=1)
        logo_face_curved_front = faces().filter_by(
            GeomType.CYLINDER).group_by(Axis.X)[-1].face()

    # Highlight text edges with a custom color
    to_highlight = logo_obj.edges().group_by(Axis.X)[-1]
    logo_obj_hl = Compound(to_highlight).translate(
        (1e-3, 0, 0))  # To avoid z-fighting
    ...
    # Remove face from the main object
    logo_obj = Compound(logo_obj.faces() - ShapeList([logo_face_curved_front]))

    # Add a logo image to the CAD part
    logo_img_location = logo_obj.faces().group_by(Axis.X)[
        0].face().center_location
    # Avoid overlapping and adjust placement
    logo_img_location *= Location((0, 0, 4e-2), (0, 0, 90))
    logo_img_path = os.path.join(ASSETS_DIR, 'img.jpg')
    img_glb_bytes, img_name = image_to_gltf(
        logo_img_path, logo_img_location, height=18)
...
if __name__ == "__main__":
    ...
    from . import export_all, image_to_gltf, show
