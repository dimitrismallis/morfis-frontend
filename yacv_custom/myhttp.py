...
FRONTEND_BASE_PATH = os.getenv(
    'FRONTEND_BASE_PATH', os.path.join(FILE_DIR, 'frontend'))
...
class HTTPHandler(SimpleHTTPRequestHandler):
        ...
        if self.directory:  # Ensure proper subdirectory
            ...
            if not os.path.abspath(path).startswith(base):
                self.send_error(HTTPStatus.FORBIDDEN,
                                "Path is not in the frontend directory")
                return ''
        return path
    ...
    def _api_updates(self):
        ...
        with self.yacv.frontend_lock.r_locked():
            ...
            subscription = self.yacv.show_events.subscribe(
                yield_timeout=1.0)  # Keep-alive interval
            try:
                for data in subscription:
                    if data is None:
                        write_chunk(':keep-alive\n\n')
                    else:
                        logger.debug('Sending info about %s: %s',
                                     data.name, data)
    ...
    def _api_object(self, obj_name: str):
        ...
        self.send_header('Content-Disposition',
                         f'attachment; filename="{obj_name}.glb"')
