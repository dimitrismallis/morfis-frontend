...
class YACV:
    ...
    def __init__(self):
        ...
        raw_protocol = os.getenv(
            'YACV_PROTOCOL', 'http' if sys.platform != 'emscripten' else 'stderr').upper()
        ...
        self.color_faces = _read_color(
            os.getenv("YACV_COLOR_FACES", "#ffbf00"))  # Default yellow
        self.color_edges = _read_color(
            os.getenv("YACV_COLOR_EDGES", "#1a1aff"))  # Default blue
        self.color_vertices = _read_color(
            os.getenv("YACV_COLOR_VERTICES", "#1a1a1a"))  # Default dark gray
        logger.info('Using yacv-server v%s', get_version())

    def start(self):
        """Starts the web server in the background"""
        if self.protocol == YACVProtocol.STDERR:
            return  # No server to start, just print to stderr
        ...
        self.server_thread = Thread(
            target=self._run_server, name='yacv_server', daemon=True)
    ...
    def stop(self, *args):
        ...
        if self.protocol == YACVProtocol.STDERR:
            return  # No server to stop, just print to stderr
        ...
        self._show_event(UpdatesApiFullData(name='__shutdown',
                         _hash='', is_remove=None, obj=None))

        # If we were too fast, ensure that at least one client has connected
        graceful_secs_connect = float(
            os.getenv('YACV_GRACEFUL_SECS_CONNECT', 12.0))
        ...
        graceful_secs_request = float(
            os.getenv('YACV_GRACEFUL_SECS_WORK', 1000000))
    ...
    def _run_server(self):
        ...
        self.server = ThreadingHTTPServer(
            (os.getenv('YACV_HOST', 'localhost'),
             int(os.getenv('YACV_PORT', 32323))),
            lambda a, b, c: HTTPHandler(a, b, c, yacv=self))
        # noinspection HttpUrlsUsage
        logger.info(
            f'Serving at http://{self.server.server_name}:{self.server.server_port}')
        ...
        if self.protocol == YACVProtocol.STDERR:
            ...
            if not event.is_remove:
                ...
                if glb_and_hash is None:
                    logger.warning(
                        'Object %s not found, ignoring it...', event.name)
                    return
                glb = glb_and_hash[0]
    ...
    def show(self, *objs: List[YACVSupported], names: Optional[Union[str, List[str]]] = None, **kwargs):
        ...
        assert len(names) == len(
            objs), 'Number of names must match the number of objects'
        for color_name in ('color_faces', 'color_edges', 'color_vertices'):
            if color_name in kwargs:
                kwargs[color_name] = get_color(
                    kwargs[color_name]) or _read_color(kwargs[color_name])
    ...
    def show_cad_all(self, **kwargs):
        ...
        self.show(*[cad for _, cad in all_cad],
                  names=[name for name, _ in all_cad], **kwargs)
    ...
    def export(self, name: str) -> Optional[Tuple[bytes, str]]:
        ...
        with self.build_events_lock:
            ...
            if name not in self.build_events:
                logger.debug('Building object %s with hash %s',
                             name, event.hash)
                ...
                if isinstance(event.obj, bytes):  # Already a GLTF
                    ...
                    gltf = tessellate(
                        event.obj,
                        color_faces=event.kwargs.get(
                            'color_faces', self.color_faces),
                        color_edges=event.kwargs.get(
                            'color_edges', self.color_edges),
                        color_vertices=event.kwargs.get(
                            'color_vertices', self.color_vertices),
                        color_obj=event.kwargs.get('color_obj', None),
                        tolerance=event.kwargs.get('tolerance', 0.1),
                        angular_tolerance=event.kwargs.get(
...
def _read_texture_uri(uri: str) -> Optional[Tuple[bytes, str]]:
    ...
    if uri.startswith("file:"):
        ...
        return data, mtype
    # https://en.wikipedia.org/wiki/Data_URI_scheme#Syntax (limited)
