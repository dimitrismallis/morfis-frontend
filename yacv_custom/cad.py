...
# Faces, Edges, Vertices and Locations for now
CADCoreLike = Union[TopoDS_Shape, TopLoc_Location]
...
def get_shape(obj: CADLike, error: bool = True) -> Optional[CADCoreLike]:
    ...
    if isinstance(obj, list) or isinstance(obj, tuple) or isinstance(obj, set) or isinstance(obj, dict):
        try:
            if isinstance(obj, dict):
                ...
                obj_iter = iter(obj)
            # print(obj, ' -> ', obj_iter)
            shapes_raw = [get_shape(sub_obj, error=False)
                          for sub_obj in obj_iter]
            # Silently drop non-shapes
            shapes_raw_filtered = [
                shape for shape in shapes_raw if shape is not None]
            if len(shapes_raw_filtered) > 0:  # Continue if we found at least one shape
                # Sorting is required to improve hashcode consistency
                shapes_raw_filtered_sorted = sorted(
                    shapes_raw_filtered, key=lambda x: _hashcode(x))
    ...
    if error:
        raise ValueError(
            f'Cannot show object of type {type(obj)} (submit issue?)')
    else:
        return None
...
def image_to_gltf(source: str | bytes, center: any, width: Optional[float] = None, height: Optional[float] = None,
    ...
    if save_mime == 'image/jpeg':
        ...
        raise ValueError(
            f'Unsupported save MIME type (for GLTF files): {save_mime}')
    ...
    if width is None and height is None:
        # Fallback to pixels == mm?
        raise ValueError('At least one of width or height must be specified')
    elif width is None:
        width = img.width / img.height * height
    elif height is None:
        # Apply default aspect ratio if unspecified
        height = height or img.height / img.width * width
