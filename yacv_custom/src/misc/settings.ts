...
export const settings = (async () => {
    let settings = {
        ...
        // Uniform (1x1 pixel) light gray background for visibility (following dark/light mode)
        skybox: (window.matchMedia("(prefers-color-scheme: dark)").matches ?
            ...
            "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAIAAACQd1PeAAAADElEQVR4nGP4+vUrAAXCAuCuRnaIAAAAAElFTkSuQmCC"),
    ...
    for (let i = 0; i < settings.preload.length; i++) {
        ...
        if (url === '<auto>') {
            ...
            const possibleBackend = new URL("./?api_updates=true&refresh=" + Date.now(), window.location.href)
