...
// Type definition for MObject3D (originally from Selection.vue)
export type MObject3D = any;
...
export class SelectionInfo {
    ...
    /** The geometry index that corresponds to build123d faces()/edges()/vertices() array */
    geometryIndex: number | null;

    constructor(object: MObject3D, kind: "face" | "edge" | "vertex", indices: [number, number], geometryIndex: number | null = null) {
        this.object = object;
        this.kind = kind;
...
export function hitToSelectionInfo(hit: Intersection<MObject3D>): SelectionInfo | null {
    ...
    if (kind == "Mesh" || kind == "SkinnedMesh") {
        ...
        let faceIndex = getFaceIndex(hit);
        return new SelectionInfo(hit.object, "face", indices, faceIndex);
        ...
        if (indices === null) return null;
        let edgeIndex = getEdgeIndex(hit);
        return new SelectionInfo(hit.object, "edge", indices, edgeIndex);
    } else if (kind == "Points") {
        if (hit.index === undefined) return null;
        let vertexIndex = getVertexIndex(hit);
        return new SelectionInfo(hit.object, "vertex", [hit.index, hit.index + 1], vertexIndex);
    }
    return null;
}
...
// Calculate face index from triangle indices
function getFaceIndex(hit: Intersection<MObject3D>): number | null {
    let faceTrianglesEnd = hit?.object?.geometry?.userData?.face_triangles_end;
    if (!hit.faceIndex || !faceTrianglesEnd) return null;

    let rawIndex = hit.faceIndex * 3; // Faces are triangles with 3 indices
    for (let i = 0; i < faceTrianglesEnd.length; i++) {
        let faceSwapIndex = faceTrianglesEnd[i];
        if (rawIndex < faceSwapIndex) {
            return i; // This is the face index in the build123d faces() array
        }
    }
    return null;
}
...
function hitEdgePointIndices(hit: Intersection<MObject3D>): [number, number] | null {
    ...
    let rawIndex = hit.index; // Index of the point along the edge
...
// Calculate edge index from point indices
function getEdgeIndex(hit: Intersection<MObject3D>): number | null {
    let edgePointsEnd = hit?.object?.geometry?.userData?.edge_points_end;
    if (!edgePointsEnd || hit.index === undefined) return null;

    let rawIndex = hit.index;
    for (let i = 0; i < edgePointsEnd.length; i++) {
        let edgeSwapIndex = edgePointsEnd[i];
        if (rawIndex < edgeSwapIndex) {
            return i; // This is the edge index in the build123d edges() array
        }
    }
    return null;
}

// Calculate vertex index - vertices are processed sequentially in build123d vertices() order
function getVertexIndex(hit: Intersection<MObject3D>): number | null {
    if (hit.index === undefined) return null;
    // For vertices, the hit.index directly corresponds to the vertex index in build123d vertices() array
    return hit.index;
}
